...
int main(int argc, char *argv[])
{
  ...
  uint32_t *pGPIOC_ODR   = (uint32_t *)STM32_GPIOC_ODR;

  /* Habilita clock GPIOA e GPIOC */

  reg  = *pRCC_AHB1ENR;
  reg |= (RCC_AHB1ENR_GPIOAEN | RCC_AHB1ENR_GPIOCEN);
  *pRCC_AHB1ENR = reg;
  ...
  while(1)
    {
      ...
      if(!(reg &(1<<0))){ // Se botão for apertado
        contador = 1;
      }

      if(contador == 1){
        if(*pGPIOC_ODR &(1<<13)){ // Se LED estiver desligado
